# -*- coding: utf-8 -*-
# на базе статьи http://habrahabr.ru/post/188152/
...
class Stribog512:
    ...
    # Таблица подстановок для преобразования S
    __Sbox = [
    ...
    # Таблица констант для функции KeySchedule
    __C = [
        [
    ...
    # Сложение двух массивов по модулю 2**512
    ...
    # Поэлементный XOR двух массивов
    ...
    # Линейное биективное преобразование множества двоичных векторов
    def __L(self, state):
        result = [0] * 64
        for i in range(0, 8):
            t = 0
    ...
    # Подсчет хэша
    # message - list байт - сообщение
    # no_pad  - bool - флаг паддинга (если установлен, то при длине сообщения кратной 512 бит не будет дополнительной итерации)
    # return  - list байт - хэш
    def GetHash(self, message, no_pad=False):
        # Этап 1
        N = [0] * 64
        Sigma = [0] * 64
        h = [0] * 64  # для 512-битного выхода h = IV = 0^512
        ...
        while (length_bits >= 512):
            tmp = message[length - i * 64: length - (i - 1) * 64]
        ...
        # Этап 3
        last = message[0: length - (i - 1) * 64]

        # Если осталось 0 бит в сообщении и выстален флаг no_pad,
